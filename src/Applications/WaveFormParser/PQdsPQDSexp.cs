    ...
    public partial class PQdsPQDSexp : Form
    {
        ...
        private void PQioPQDSexp_Load(object sender, EventArgs e)
        {
            ...
            using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
            {
                ...
                List<PQio.Model.Asset> assets = assetTable.QueryRecordsWhere("(SELECT COUNT(DataSeries.ID) FROM DataSeries LEFT OUTER JOIN Channel ON Channel.ID = DataSeries.ChannelID WHERE Channel.AssetID = Asset.ID) > 0  ").ToList();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            List<Tuple<PQio.Model.Event, PQio.Model.Asset>> files = new List<Tuple<PQio.Model.Event, Model.Asset>>();
            ...
            using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
            {
                ...
                foreach (int assetID in selectedAssetIDs)
                {
                    ...
                    foreach(int eventID in selectedEvtIDs)
                    {
                        ...
                        {
                            files.Add(new Tuple<PQio.Model.Event, Model.Asset>(eventTable.QueryRecordWhere("ID = {0}", eventID), assetTable.QueryRecordWhere("ID = {0}", assetID)));
                        }
                    }
        ...
        {
            chLstBoxEvt.BeginUpdate();

            for (int i = 0; i < chLstBoxEvt.Items.Count; i++)
            {
                chLstBoxEvt.SetItemCheckState(i, CheckState.Checked);
            }

            chLstBoxEvt.EndUpdate();
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < chLstBoxEvt.Items.Count; i++)
            {
                chLstBoxEvt.SetItemCheckState(i, CheckState.Unchecked);
            }
        ...
        {
            chLstBoxAsset.BeginUpdate();

            for (int i = 0; i < chLstBoxAsset.Items.Count; i++)
            {
                chLstBoxAsset.SetItemCheckState(i, CheckState.Checked);
            }

            chLstBoxAsset.EndUpdate();
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < chLstBoxAsset.Items.Count; i++)
            {
                chLstBoxAsset.SetItemCheckState(i, CheckState.Unchecked);
            }
