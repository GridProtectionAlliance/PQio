    ...
    public partial class PQdsMain : Form
    {
        ...
        private List<TreeNode> GetWaveFormMetaData()
        {
            ...
            using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
            {
                ...
                {
                    GSF.Data.Model.TableOperations<PQio.Model.SignalType> signalTypeTable = new GSF.Data.Model.TableOperations<PQio.Model.SignalType>(connection);
                    int? signalType = channelTable.QueryRecords().First().SignalType;

                    if (signalType != null)
                    {
                        TreeNode datatype = new TreeNode(string.Format("Data Type: {0} ({1})", PQio.Model.SignalType.ToDisplay((int)signalType), signalType));
                ...
                if (DataSensitivity.CodeisGlobal())
                {
                    ...
                    TreeNode datasensitivity = new TreeNode(string.Format("Data Sensitivity: {0} ({1})", PQio.Model.DataSensitivityCode.ToDisplay((int)sesitivity), sesitivity));
        ...
        private TreeNode AssetToNode(PQio.Model.Asset asset)
        {
        ...
        private TreeNode DeviceToNode(PQio.Model.Meter device)
        {
            TreeNode node;
            ...
            {
                node.Nodes.Add(string.Format("Event Type: {0} ({1})", PQio.Model.ConnectionType.ToDisplay(device.ConnectionType), device.ConnectionType));
            }
        ...
        private TreeNode EventToNode(PQio.Model.Event evt)
        {
            ...
            {
                node.Nodes.Add(string.Format("Event Type: {0} ({1})", PQio.Model.EventType.ToDisplay(evt.EventType), evt.EventType));
            }
            if (evt.FaultType != null)
            {
                node.Nodes.Add(string.Format("Fault Type: {0} ({1})", PQio.Model.FaultType.ToDisplay(evt.FaultType), evt.FaultType));
            }
            ...
            {
                node.Nodes.Add(string.Format("Fault Cause: {0} ({1})", PQio.Model.FaultCause.ToDisplay(evt.FaultCause), evt.FaultCause));
            }
        ...
        private TreeNode ChannelList(PQio.Model.Asset asset)
        {
        ...
        private TreeNode ChannelNode(PQio.Model.Channel channel)
        {
            ...
            using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
            {
                ...
                PQio.Model.Meter meter = meterTable.QueryRecordWhere("ID = {0}", channel.MeterID);
        ...
        private TreeNode EvtNode(PQio.Model.DataSeries dataSeries)
        {
            using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
            {
                ...
                PQio.Model.Event evt = evtTable.QueryRecordWhere("ID = {0}", dataSeries.EventID);
        ...
        private void UpdateChannelMetaData(PQio.Model.Channel channel)
        {
            ...
            using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
            {
                ...
                {
                    TreeNode datatype = new TreeNode(string.Format("Data Type: {0} ({1})", PQio.Model.SignalType.ToDisplay((int)channel.SignalType), channel.SignalType));
                }
                if (channel.MeasurementType != MeasurementType.other)
                {
        ...
        private void AddToChart(PQio.Model.DataSeries dataseries, Chart chart)
        {
            Series chartSeries;
